...
const BG_STORAGE_KEY = "oracle-bg";
 ...
 *  inherit the `:root` default from `globals.css`; when omitted we also
...
export function ThemeProvider({ children }: { children: ReactNode }) {
   ...
   *  built-ins; the API call below merges in user themes (no-op here). */
    ...
    api
      ...
      .then((resp) => {
        ...
        if (resp.active) {
          ...
          // migrated value back so it converges too.
