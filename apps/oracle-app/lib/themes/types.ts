 ...
 *
 * Ported verbatim from the desktop Hermes dashboard
 * (`/usr/local/lib/hermes-agent/web/src/themes/types.ts`). Do not rewrite the
 * model — slices depend on the exact `DashboardTheme` shape.
 *
 ...
 *                         design-system cascade in `app/globals.css` derives
 ...
 *  `globals.css` (Hermes-teal `#ffe6cb` for input, `#34d399` for output).
 *
...
export interface ThemeListEntry {
  ...
  label: string;
  name: string;
  /** Full theme definition. Present for user-defined themes; undefined for
