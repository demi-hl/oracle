...
test("builder fees are absent without configuration and caller-supplied holder flags are ignored", async () => {
  ...
  assert.equal(holder.holderExempt, false);
  assert.equal(holder.holderVerificationStatus, "wallet-required");
  assert.equal(holder.builderApprovalRequired.status, "wallet-required");
});
...
test("an unavailable holder check fails closed to the standard approved builder fee", async () => {
  const o = await hlPreparePerpOrder(
    { coin: "BTC", side: "buy", type: ORDER_TYPES.LIMIT, price: "60000", size: "0.01", user: USER },
    {
      ...stubWithBuilderApproval(20),
  ...
  assert.deepEqual(o.action.builder, { b: BUILDER, f: 20 });
  assert.equal(o.builderFee.bps, 2);
